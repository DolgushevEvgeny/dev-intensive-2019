...
object Utils {
    ...
    fun transliteration(payload: String, divider: String = " "): String {
        ...
        for (char in payload.trim())
            result += processChar(char, map)

        return result.replace(" ", divider)
    }

    private fun processChar(char: Char, map: HashMap<Char, String>): String {
        val ch = map[char.toLowerCase()] ?: char.toString()
        return if (char.isLowerCase()) ch
        else ch.capitalize()
    }
